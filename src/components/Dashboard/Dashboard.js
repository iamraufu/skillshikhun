    ...
    return (
        <section className='dashboard-container'>
            ...
            , minHeight: '100vh' 
                ...
                {JSON.parse(localStorage?.user)?.name || null}</span></h1> */}

                    {
                        userPhoneData?.phone ?
                            <h1 style={{ 
                                // paddingTop: '4rem', 
                                fontSize: '24px', lineHeight: '36px', color: '#343b6d', fontWeight: '700' }} className=''>স্বাগতম, <span style={{ fontSize: '24px', lineHeight: '36px', color: '#b94a8f', fontWeight: '600' }}>
                                {userPhoneData.name}
                            </span></h1>
                            :
                            <h1 style={{ paddingTop: '4rem', fontSize: '24px', lineHeight: '36px', color: '#343b6d', fontWeight: '700' }} className=''>লোড হচ্ছে...<span style={{ fontSize: '24px', lineHeight: '36px', color: '#b94a8f', fontWeight: '600' }}>
                            </span></h1>
                    }

                    <h2 style={{ fontSize: '18px', lineHeight: '31px', color: '#495082', fontWeight: '500' }}>ক্লাসের খুঁটিনাটি সব জেনে নিন আপনার <span style={{ color: '#653dae' }} className='fw-bold'>Skill শিখুন</span> এর ড্যাশবোর্ডে</h2>
                    {/* <button onClick={() => logOut()} className='btn btn-danger'>Log Out</button> */}

                    {
                        demoClasses?.length > 0 ? 
                        <h2 style={{ fontSize: '24px', lineHeight: '36px', color: '#343b6d', fontWeight: '700' }} className=' my-3'>You have total <span className='text-primary'>{demoClasses.length}</span> Free Classes</h2> :
                        <h1 style={{ paddingTop: '1rem', fontSize: '24px', lineHeight: '36px', color: '#343b6d', fontWeight: '700' }} className=''>কোনো ক্লাস নেই<span style={{ fontSize: '24px', lineHeight: '36px', color: '#b94a8f', fontWeight: '600' }}>
                            </span></h1>
                    }
                    {
                        demoClasses.length?     
                        demoClasses.map((demoClass, index) => 
                        <li key={index}>{demoClass.class_date} - <span style={{color:'#b94a8f', fontWeight:'700'}}>{demoClass.category}</span> - {demoClass.class_time}</li> 
                    ...
                    {/* {
                                    demoClasses?.phone ? demoClasses.map(demoClass => (
                                    <li>{demoClass.category}</li>
                                    )):
                    ...
                    <div className="row py-5 m-2">
                                    ...
                                    <div style={{ backgroundColor: 'rgb(53,72,149)', borderRadius: '15px', top: '120px', height: '320px' }} className="col-xl-2 col-lg-3 col-md-4 col-sm-4 mt-3 position-sticky">
                                        ...
                                        <ul style={{ listStyle: 'none', fontSize: '18px', lineHeight: '31px', fontWeight: '500' }} className='text-white'>
                                            <li className='pt-4 py-2'><Link onClick={() => { window.scrollTo(0, 0); }} to='/' className='text-decoration-none text-white'>হোম</Link></li>
